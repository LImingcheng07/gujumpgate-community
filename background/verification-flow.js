(function attachBackgroundVerificationFlow(root, factory) {
  ...
  function createVerificationFlowHelpers(deps = {}) {
    const {
      ...
      DUCKDUCKGO_MAIL_PROVIDER = 'duckduckgo-mail',
      ...
      requestDuckDuckGoManualVerificationCodeRequest,
      sendToContentScript,
    ...
    async function pollFreshVerificationCode(step, state, mail, pollOverrides = {}) {
      ...
      if (mail.provider === CLOUDFLARE_TEMP_EMAIL_PROVIDER) {
        ...
        const verificationMode = String(state?.duckDuckGoVerificationMode || '').trim().toLowerCase();
        // When DDG is the email generator with manual verification, show the dialog instead of polling CF
        if (emailGenerator === DUCKDUCKGO_MAIL_PROVIDER && verificationMode === 'manual') {
          if (typeof requestDuckDuckGoManualVerificationCodeRequest === 'function') {
            const response = await requestDuckDuckGoManualVerificationCodeRequest(step, {
              label: getVerificationCodeLabel(step),
            });
            if (response?.error) {
              throw new Error(response.error);
            }
            const manualCode = String(response?.code || '').trim();
            if (!manualCode) {
              throw new Error(`步骤 ${step}：已取消手动${getVerificationCodeLabel(step)}验证码输入。`);
            }
            return {
              ok: true,
              code: manualCode,
        ...
        // When DDG is the generator (auto mode), use its forwarding mailbox as the CF receive mailbox
        const resolvedState = emailGenerator === DUCKDUCKGO_MAIL_PROVIDER
          ? { ...state, cloudflareTempEmailReceiveMailbox: String(state?.duckDuckGoForwardingMailbox || '').trim() }
          : state;
        const timedPoll = await applyMailPollingTimeBudget(step, {
          ...getVerificationPollPayload(step, state),
          ...cleanPollOverrides,
        }, cleanPollOverrides, `轮询${getVerificationCodeLabel(step)}验证码邮箱`);
        return pollCloudflareTempEmailVerificationCode(step, resolvedState, timedPoll.payload);
      }
      ...
      if (mail.provider === DUCKDUCKGO_MAIL_PROVIDER) {
        if (String(state?.duckDuckGoVerificationMode || '').trim().toLowerCase() !== 'manual') {
          const timedPoll = await applyMailPollingTimeBudget(step, {
            ...getVerificationPollPayload(step, state),
            ...cleanPollOverrides,
            targetEmail: String(state?.duckDuckGoForwardingMailbox || '').trim() || cleanPollOverrides.targetEmail,
          }, cleanPollOverrides, `轮询${getVerificationCodeLabel(step)}验证码邮箱`);
          return pollCloudflareTempEmailVerificationCode(step, {
            ...state,
            mailProvider: CLOUDFLARE_TEMP_EMAIL_PROVIDER,
            cloudflareTempEmailReceiveMailbox: String(state?.duckDuckGoForwardingMailbox || '').trim(),
          }, timedPoll.payload);
        }
        if (typeof requestDuckDuckGoManualVerificationCodeRequest === 'function') {
          const response = await requestDuckDuckGoManualVerificationCodeRequest(step, {
