...
importScripts(
  ...
  'background/duckduckgo-mail-provider.js',
  'icloud-utils.js',
...
const DUCKDUCKGO_MAIL_GENERATOR = 'duckduckgo-mail';
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  emailGenerator: 'duck',
  duckDuckGoApiToken: '',
  duckDuckGoVerificationMode: 'auto',
  duckDuckGoForwardingMailbox: '',
...
function normalizeEmailGenerator(value = '') {
  ...
  if (normalized === DUCKDUCKGO_MAIL_GENERATOR) return DUCKDUCKGO_MAIL_GENERATOR;
  return 'duck';
}
...
const duckDuckGoMailProvider = self.MultiPageBackgroundDuckDuckGoMailProvider.createDuckDuckGoMailProvider({
  addLog,
  getState,
  persistRegistrationEmailState,
  setEmailState,
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'mailProvider':
      {
        ...
        if (normalizedMailProvider === DUCKDUCKGO_MAIL_PROVIDER) {
          return DUCKDUCKGO_MAIL_PROVIDER;
        }
    ...
    case 'duckDuckGoApiToken':
      return String(value || '');
    case 'duckDuckGoVerificationMode':
      return normalizeDuckDuckGoVerificationMode(value);
    case 'duckDuckGoForwardingMailbox':
      return normalizeDuckDuckGoForwardingMailbox(value);
...
function getEmailGeneratorLabel(generator) {
  ...
  if (generator === DUCKDUCKGO_MAIL_GENERATOR) return 'DuckDuckGo Email Protection';
  return 'Duck 邮箱';
}
...
async function fetchGeneratedEmail(state, options = {}) {
  ...
  if (generator === DUCKDUCKGO_MAIL_GENERATOR) {
    return fetchDuckDuckGoMailAddress(currentState, options);
  }
...
const verificationFlowHelpers = self.MultiPageBackgroundVerificationFlow?.createVerificationFlowHelpers({
  ...
  requestDuckDuckGoManualVerificationCodeRequest: (step, payload = {}) => chrome.runtime.sendMessage({
    type: 'REQUEST_DUCKDUCKGO_VERIFICATION_CODE',
...
function getMailConfig(state) {
  ...
  if (provider === DUCKDUCKGO_MAIL_PROVIDER) {
    return { provider: DUCKDUCKGO_MAIL_PROVIDER, label: 'DuckDuckGo Email Protection' };
  }
  if (provider === '163') {
