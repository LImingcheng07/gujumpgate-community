...
const duckDuckGoMailSection = document.getElementById('duckduckgo-mail-section');
...
const rowDuckDuckGoMailApiToken = document.getElementById('row-duckduckgo-mail-api-token');
const rowDuckDuckGoMailVerificationMode = document.getElementById('row-duckduckgo-mail-verification-mode');
const rowDuckDuckGoMailForwardingMailbox = document.getElementById('row-duckduckgo-mail-forwarding-mailbox');
const inputDuckDuckGoMailApiToken = document.getElementById('input-duckduckgo-mail-api-token');
const inputDuckDuckGoMailForwardingMailbox = document.getElementById('input-duckduckgo-mail-forwarding-mailbox');
const duckDuckGoMailVerificationModeButtons = Array.from(document.querySelectorAll('[data-duckduckgo-mail-verification-mode]'));
...
const BUILTIN_PLUS_CHECKOUT_CLOUD_CONVERSION_API_URL = 'https://your-checkout-converter.example/api/checkout';
...
const GUIDE_REPOSITORY_URL = 'https://github.com/mingcheng/gujumpgate-community';
...
const DUCKDUCKGO_MAIL_PROVIDER = 'duckduckgo-mail';
...
const MAIL_PROVIDER_LOGIN_CONFIGS = {
  ...
  [DUCKDUCKGO_MAIL_PROVIDER]: {
    label: 'DuckDuckGo Email Protection',
...
function normalizeDuckDuckGoForwardingMailboxValue(value = '') {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized) return '';
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized) ? normalized : '';
}

function setDuckDuckGoVerificationMode(value = '') {
  const selected = normalizeDuckDuckGoVerificationMode(value);
  duckDuckGoMailVerificationModeButtons.forEach((button) => {
    if (!button) return;
    const mode = normalizeDuckDuckGoVerificationMode(button.dataset.duckduckgoMailVerificationMode || '');
    button.classList.toggle('is-active', mode === selected);
    button.setAttribute('aria-pressed', mode === selected ? 'true' : 'false');
  });
}

function getSelectedDuckDuckGoVerificationMode() {
  const activeButton = duckDuckGoMailVerificationModeButtons.find((button) => button?.classList.contains('is-active'));
  return normalizeDuckDuckGoVerificationMode(activeButton?.dataset.duckduckgoMailVerificationMode || '');
}
...
function applyDuckDuckGoMailSettingsState(state = {}) {
  if (inputDuckDuckGoMailApiToken) {
    inputDuckDuckGoMailApiToken.value = state?.duckDuckGoApiToken || '';
  }
  if (inputDuckDuckGoMailForwardingMailbox) {
    inputDuckDuckGoMailForwardingMailbox.value = state?.duckDuckGoForwardingMailbox || '';
  }
  setDuckDuckGoVerificationMode(state?.duckDuckGoVerificationMode);
}
...
function collectSettingsPayload() {
  ...
  const normalizeDuckDuckGoForwardingMailboxInput = typeof normalizeDuckDuckGoForwardingMailboxValue === 'function'
    ? normalizeDuckDuckGoForwardingMailboxValue
  ...
  return {
    ...
    duckDuckGoApiToken: (typeof inputDuckDuckGoMailApiToken !== 'undefined' && inputDuckDuckGoMailApiToken) ? inputDuckDuckGoMailApiToken.value : '',
    duckDuckGoVerificationMode: getSelectedDuckDuckGoVerificationMode(),
    duckDuckGoForwardingMailbox: normalizeDuckDuckGoForwardingMailboxInput((typeof inputDuckDuckGoMailForwardingMailbox !== 'undefined' && inputDuckDuckGoMailForwardingMailbox) ? inputDuckDuckGoMailForwardingMailbox.value : ''),
...
function applySettingsState(state) {
  ...
  setMail2925Mode(state?.mail2925Mode);
  {
    ...
    if (restoredMailProvider === cloudflareTempEmailProvider) {
      selectEmailGenerator.value = restoredEmailGenerator === DUCKDUCKGO_MAIL_PROVIDER
        ? DUCKDUCKGO_MAIL_PROVIDER
        : 'cloudflare-temp-email';
    ...
    } else if (restoredEmailGenerator === DUCKDUCKGO_MAIL_PROVIDER) {
      selectEmailGenerator.value = DUCKDUCKGO_MAIL_PROVIDER;
    } else {
      selectEmailGenerator.value = 'duck';
    }
  }
  applyDuckDuckGoMailSettingsState(state || {});
...
function getSelectedEmailGenerator() {
  ...
  if (generator === DUCKDUCKGO_MAIL_PROVIDER) return DUCKDUCKGO_MAIL_PROVIDER;
  return 'duck';
}
...
function getEmailGeneratorUiCopy() {
  ...
  if (getSelectedEmailGenerator() === DUCKDUCKGO_MAIL_PROVIDER) {
    return {
      buttonLabel: '生成',
      placeholder: '点击生成 DuckDuckGo Email Protection 邮箱，或手动粘贴邮箱',
      successVerb: '生成',
      label: 'DuckDuckGo Email Protection',
    };
  }
...
function normalizeDuckDuckGoVerificationCodeInputValue(value = '') {
  return String(value || '').trim().replace(/[^\da-zA-Z-]/g, '');
}

async function openDuckDuckGoVerificationCodeDialog(payload = {}) {
  if (!sharedFormDialog?.open) {
    throw new Error('验证码输入弹窗未加载，请刷新扩展后重试。');
  }

  const step = Math.max(1, Math.floor(Number(payload?.step) || 0));
  const label = String(payload?.label || (step === 4 ? '注册' : '登录')).trim() || '登录';
  const initialCode = normalizeDuckDuckGoVerificationCodeInputValue(payload?.code || '');
  const result = await sharedFormDialog.open({
    title: `输入 DuckDuckGo ${label}验证码`,
    message: `请填写当前 DuckDuckGo Email Protection 邮件中的${label}验证码，确认后插件会继续自动提交。`,
    confirmLabel: '提交验证码',
    confirmVariant: 'btn-primary',
    fields: [
      {
        ...
        value: initialCode,
        validate: (value) => {
          const normalized = normalizeDuckDuckGoVerificationCodeInputValue(value);
  ...
  const code = normalizeDuckDuckGoVerificationCodeInputValue(result?.code || '');
  if (!code) {
    return { cancelled: true, code: '' };
  }
  return { code };
...
function updateMailProviderUI() {
  ...
  const useDuckDuckGoMailProvider = selectMailProvider.value === DUCKDUCKGO_MAIL_PROVIDER;
  const gmailAliasGenerator = typeof GMAIL_ALIAS_GENERATOR === 'string'
    ? GMAIL_ALIAS_GENERATOR
    : 'gmail-alias';
  const customEmailPoolGenerator = typeof CUSTOM_EMAIL_POOL_GENERATOR === 'string'
  ...
  const allowedEmailGenerators = useHotmail || useLuckmail || useCustomEmail
    ...
    : (useCloudflareTempEmailProvider
      ? new Set(['cloudflare-temp-email', DUCKDUCKGO_MAIL_PROVIDER])
  ...
  if (useCloudflareTempEmailProvider && String(selectEmailGenerator?.value || '').trim().toLowerCase() !== 'cloudflare-temp-email' && String(selectEmailGenerator?.value || '').trim().toLowerCase() !== DUCKDUCKGO_MAIL_PROVIDER) {
    selectEmailGenerator.value = 'cloudflare-temp-email';
  }
  ...
  const useDuckDuckGoMailGenerator = selectedGenerator === DUCKDUCKGO_MAIL_PROVIDER;
  ...
  const showDuckDuckGoMailSettings = useEmailGenerator && useDuckDuckGoMailGenerator;
  ...
  const showDuckDuckGoForwardingMailbox = showDuckDuckGoMailSettings
    && getSelectedDuckDuckGoVerificationMode() === 'auto';
  ...
  if (duckDuckGoMailSection) {
    duckDuckGoMailSection.style.display = showDuckDuckGoMailSettings ? '' : 'none';
  }
  ...
  if (rowDuckDuckGoMailApiToken) rowDuckDuckGoMailApiToken.style.display = showDuckDuckGoMailSettings ? '' : 'none';
  if (rowDuckDuckGoMailVerificationMode) rowDuckDuckGoMailVerificationMode.style.display = showDuckDuckGoMailSettings ? '' : 'none';
  if (rowDuckDuckGoMailForwardingMailbox) rowDuckDuckGoMailForwardingMailbox.style.display = showDuckDuckGoForwardingMailbox ? '' : 'none';
  if (icloudSection) {
  ...
  selectEmailGenerator.disabled = useHotmail
    || useLuckmail
    || useCustomEmail
    || (useCloudflareTempEmailProvider && (!allowedEmailGenerators || allowedEmailGenerators.size <= 1))
    || useCloudMailProvider
    || useDuckDuckGoMailProvider
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'REQUEST_DUCKDUCKGO_VERIFICATION_CODE': {
      (async () => {
        const result = await openDuckDuckGoVerificationCodeDialog(message.payload || {});
        sendResponse(result || { cancelled: true, code: '' });
      })().catch((err) => {
        sendResponse({ error: err.message });
      });
...
duckDuckGoMailVerificationModeButtons.forEach((button) => {
  button?.addEventListener('click', () => {
    const mode = button.dataset.duckduckgoMailVerificationMode || '';
    setDuckDuckGoVerificationMode(mode);
    updateMailProviderUI();
    scheduleAutoSave();
  });
});
